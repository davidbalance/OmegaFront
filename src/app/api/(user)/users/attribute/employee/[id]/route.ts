...
export async function GET(
    ...
    try {
        const attribute: GetUserAttributeResponseDto = await omega().addParams({ id: params.id }).execute('userAttributeEmployeeOfDetails');
        return NextResponse.json(attribute, { status: 200 });
    } catch (error) {
        console.error(error);
        if (error instanceof ApiClientError) {
            if (error.status === 404) return NextResponse.json({}, { status: 200 });
...
export async function PATCH(
    ...
    try {
        ...
        await omega().addParams({ id: params.id }).addBody(data).execute('userAttributeEmployeeOfUpdate');
