...
export async function GET(
    ...
    try {
        ...
        const attribute: GETUserAttributeRequestDto = await getAttribute(endpoints.USER.USER.ATTRIBUTES.EMPLOYEE.FIND(params.id), { cacheExpirationSeconds: 30 });
...
export async function PATCH(
    ...
    try {
        ...
        await patchAttribute(endpoints.USER.USER.ATTRIBUTES.EMPLOYEE.FIND_ONE_AND_UPDATE(params.id), { body: data });
        return NextResponse.json({}, { status: 200 });
    } catch (error) {
        if (error instanceof FetchError) {
            return NextResponse.json({ message: error.message, data: error.data }, { status: error.response.status });
